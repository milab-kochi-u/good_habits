...
(async() => {
    ...
    */
    ...
    if(process.argv[2] == "2020-01-01 00:00:00"){
        // ユーザ「太郎」がワーク「英語学習」を追加した．
        await Taro.addWork(Learn_English);
    }
    if(process.argv[2] == "2020-01-05 00:00:00"){
        ...
        const task = await Taro.createTask({
            work : Learn_English,
            start_time : dayjs(process.argv[2]).add(10,'m'),
            end_time : dayjs(process.argv[2]).add(1,'h').add(10,'m')
        });
        await task.close(true,dayjs(process.argv[2]).add(6,'m'),models.now_time.add(1,'h').add(10,'m'));
    }
