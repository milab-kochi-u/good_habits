...
(async() => {
    ...
    */
    const Taro = user1; //await User.findOne({ where: { id: 1 }});
    const Learn_English = work3;    //await Work.findOne({ where: { id: 3 }});
    ...
    await Taro.addScheme({
        work: Learn_English,
        scheme: scheme1 //await Scheme.findOne({where:{id:1}})
    });
    ...
    task.reschedule(dayjs().add(10,'y'), dayjs().add(10,'y'));

    const taroTasks = await Taro.getTasks(Learn_English);
    console.log(taroTasks[0]);
    console.log(taroTasks[0].UsersWork.User.name);
    console.log(taroTasks[0].UsersWork.Work.label);

    const test = await models.Task.findAll({
        include: { all: true, nested: true }
        // include: [
        //     { model: models.UsersWork, include: [ models.User, models.Work ] }
        // ]
    });
    console.log(test[0]);
    console.log(test[0].UsersWork.User.name);
    console.log(test[0].UsersWork.Work.label);
